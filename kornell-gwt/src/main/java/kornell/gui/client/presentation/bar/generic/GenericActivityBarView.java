...
public class GenericActivityBarView
	...
	private static final String IMAGES_PATH = constants.imagesPath()
			+ "southBar/";
	...
	public GenericActivityBarView(EventBus bus, KornellSession session,
	...
	private void displayUser(UserInfoTO user) {
		...
		for (EnrollmentTO enrollmentTO : user.getEnrollmentsTO()
			...
			if (enrollment.getUUID().equals(
					((ClassroomPlace) placeCtrl.getWhere()).getEnrollmentUUID())
					&& EnrollmentState.enrolled.equals(enrollment
							.getState())) {
				isEnrolled = true;
				break;
			}
	...
	private void display() {
		...
		iconPrevious = new Image(IMAGES_PATH + getItemName(BUTTON_PREVIOUS)
				+ ".png");
		...
		displayButton(btnDetails, showDetails ? constants.course()
				: BUTTON_DETAILS, new Image(IMAGES_PATH
				+ getItemName(BUTTON_DETAILS) + ".png"));

		displayButton(btnNotes, BUTTON_NOTES, new Image(IMAGES_PATH
				+ getItemName(BUTTON_NOTES) + ".png"));
	...
	private void updateProgressBarPanel(Integer currentPage,
			Integer totalPages, Integer progressPercent) {
	...
	private void updateProgressBarPanel() {
		...
		if (showDetails) {
			...
			pagePanel.add(createSpan(currentPage == 0 ? "-" : "" + currentPage,
					true));
	...
	private void displayButton(final FocusPanel btn, final String buttonType,
			Image icon) {
		displayButton(btn, buttonType, icon, false);
	}

	private void displayButton(final FocusPanel btn, final String buttonType,
			Image icon, boolean invertIcon) {
		btn.clear();
	...
	void handleClickBtnNotes(ClickEvent e) {
		if (notesPopup == null) {
			notesPopup = new NotesPopup(session, Dean
					.getInstance().getCourseClassTO().getCourseClass()
					.getUUID(), Dean.getInstance().getCourseClassTO()
					.getEnrollment().getNotes());
			notesPopup.show();
		} else {
			notesPopup.show();
		}
	...
	private void enableButton(String btn, boolean enable) {
		if (BUTTON_NEXT.equals(btn)) {
			if (enable && !showDetails) {
				...
				iconNext.setUrl(IMAGES_PATH + getItemName(BUTTON_NEXT)
						+ "Disabled.png");
				btnNext.addStyleName("disabled");
			}
		} else if (BUTTON_PREVIOUS.equals(btn)) {
			if (enable && !showDetails) {
				iconPrevious.setUrl(IMAGES_PATH + getItemName(BUTTON_PREVIOUS)
						+ ".png");
	...
	public void onProgress(ProgressEvent event) {
		...
		if (event.getCurrentPage() != 0) {
			updateProgressBarPanel(event.getCurrentPage(),
					event.getTotalPages(), event.getProgressPercent());
		...
		shouldShowActivityBar = isEnrolled
				&& Dean.getInstance().getCourseClassTO() != null
				&& !CourseClassState.inactive.equals(Dean.getInstance()
						.getCourseClassTO().getCourseClass().getState());
		bus.fireEvent(
				new HideSouthBarEvent(!shouldShowActivityBar));
		this.setVisible(shouldShowActivityBar);
	}
	...
	void handleClickBtnDetails(ClickEvent e) {
		if (shouldShowActivityBar)
			bus.fireEvent(
					new ShowDetailsEvent(!showDetails));
	}
	...
	public void onShowDetails(ShowDetailsEvent event) {
		...
		if (showDetails) {
			btnDetails.addStyleName("btnSelected");
			displayButton(btnDetails, constants.course(), new Image(IMAGES_PATH
					+ getItemName(BUTTON_DETAILS) + ".png"));
		} else {
			btnDetails.removeStyleName("btnSelected");
			displayButton(btnDetails, BUTTON_DETAILS, new Image(IMAGES_PATH
					+ getItemName(BUTTON_DETAILS) + ".png"));
		}
