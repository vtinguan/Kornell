...
public class NamingConventionSequencer implements Sequencer {
	...
	public NamingConventionSequencer(EventBus bus, PlaceController ctrl,
	...
	private void warnIfNextNotOK(String nextLocation) {
		try {
			new RequestBuilder(HEAD, nextLocation).sendRequest(null,
					new RequestCallback() {
						@Override
						public void onResponseReceived(Request request,
							...
							if (nextOK) {
								bus.fireEvent(NavigationForecastEvent.Forecast.NEXT_OK
										.get());
							} else {
								bus.fireEvent(NavigationForecastEvent.Forecast.NEXT_NOT_OK
										.get());
							}
						}
						...
						public void onError(Request request, Throwable exception) {
							GWT.log("Error displaying content (onError)",
									exception);
						}
					});
	...
	private void go(final String location) {
		...
		/*
		 * try { new RequestBuilder(HEAD, location).sendRequest(null, new
		 * RequestCallback() {
		 * 
		 * @Override public void onResponseReceived(Request request, Response
		 * response) { boolean locationExists = response.getStatusCode() == 200;
		 * if (locationExists){ iframe.setSrc(location); }else
		 * if(response.getStatusCode() == 0){ //TODO: Add application warning
		 * GWT.log(
		 * "XHR cancelled, probably reject by same origin policy. Please configure cross origin resource sharing."
		 * ); iframe.setSrc(location); } else
		 * GWT.log("Error displaying content (onResponseReceived)"); }
		 * 
