...
class EnrollmentSpec extends UnitSpec with SimpleInstitution {
  ...
  "The platformAdmin" should "be able to update an enrollment" in {
    val x = "NEW NOTES"
	  val enrollment = EnrollmentsRepo.create(Entities.newEnrollment(randUUID, null, courseClass.getUUID, notAnAdmin.getUUID, null, "", EnrollmentState.requested,null,null,null))
  ...
  "A user" should "not be able to update an enrollment that doesn't belong to him" in {
    val enrollment = EnrollmentsRepo.create(Entities.newEnrollment(randUUID, null, courseClass.getUUID, institutionAdmin.getUUID, null, "", EnrollmentState.requested,null,null,null))
    val x = "NEW NOTES4"
	  val enrollmentResource = new EnrollmentResource(enrollment.getUUID)
    enrollment.setNotes(x)
