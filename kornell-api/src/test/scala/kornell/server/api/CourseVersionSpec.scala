...
class CourseVersionSpec {/* extends UnitSpec 
  ...
  var courseVersionsResource:CourseVersionsResource = ???
  ...
  "A person" should "be able to get a courseVersion" in asPlatformAdmin {
    ...
    asPerson {
      try {
        val courseVersionTO = courseVersionsResource.getCourseVersion(createdCourseVersionUUID).get
        throw new Throwable
      } catch {
  ...
  "The platformAdmin" should "be able to update a courseVersion" in asPlatformAdmin {
    val repositoryUUID :String = ??? // RepositoriesRepo().createS3Repository("", "", "", institutionUUID = institutionUUID, region = "sa-east-1").getUUID
    val name = "test name"
    val updatedCourseVersion = courseVersionsResource.getCourseVersion(courseVersionUUID).update(Entities.newCourseVersion(uuid = courseVersionUUID, repositoryUUID=repositoryUUID, courseUUID = courseUUID, name = name))
