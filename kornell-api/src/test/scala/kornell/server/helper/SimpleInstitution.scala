...
trait SimpleInstitution extends SuiteMixin with Generator with BeforeAndAfter{ this: Suite =>
  ...
  abstract override def withFixture(test: NoArgTest) = {
    ...
    institution = InstitutionsRepo.create(Entities.newInstitution(randUUID, randStr, randStr, randStr, randURL, randURL, false))
	  course = CoursesRepo.create(Entities.newCourse(randUUID, randStr, randStr, randStr, randStr))
	  courseVersion = CourseVersionsRepo.create(Entities.newCourseVersion(randUUID, randStr, course.getUUID, randUUID, new Date, randStr))
