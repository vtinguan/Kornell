...
object EmailService {
  ...
  //TODO: NICER: Fetch using ContentRepo
  private def getInstitutionLogoImage(institution: Institution): java.io.File = {
    ...
    val ittAssetsUrl = Option(institution).map(_.getAssetsURL)  
    if (!imgFile.exists && ittAssetsUrl.isDefined) Try {
      new URL(StringUtils.url(ittAssetsUrl.get, logoImageName))
    }.map { url =>
